...
| Truncated sections continuation | `The previous sections capture stopped before the end of the page and returned next_start_y: 6240. Continue from there and analyse the remaining lower portion.` | Use `analyze_page_screenshot` with `screenshot_mode: "sections"` and `start_y: 6240`. Understand that this is stateless continuation after a page reload, not a retained browser session, and inspect `first_captured_y`, coverage metadata, and warnings before assuming exact continuity. |
...
For the continuation case, specifically record whether the model uses the prior `next_start_y` as `start_y`, understands that the page is loaded again, checks the actual `first_captured_y`, and avoids inventing a continuation token, session ID, or retained browser context.
...
A result is satisfactory when the model selects the expected tool on the first attempt, supplies only supported arguments, avoids the visual tools for the textual negative control, uses stateless continuation correctly, understands the structured-output contract, and does not erase relevant operational effects behind an absolute `Risks: None` statement.
